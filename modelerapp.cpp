...
void ModelerApplication::RedrawLoop(void*)
{
    static bool up = true;
    static double i = 0;
    double diff = (double)VAL(SPEED) / (double)VAL(FPS);

    double head = VAL(HEAD_Y);
    int headx = ModelerApplication::Instance()->GetControlValue(HEAD_X);
    ...
    if (ModelerApplication::Instance()->m_animating) {
        ...
        //Flag Wings
        ModelerApplication::Instance()->SetControlValue(HEIGHT, VAL(HEIGHT)+i);
        ModelerApplication::Instance()->SetControlValue(WINGS, VAL(HEIGHT) + -i*100);

        //Move Head (Y)
        ModelerApplication::Instance()->SetControlValue(HEAD_Y, head);
        ModelerApplication::Instance()->SetControlValue(HEAD_Y, head);

        //Move Head (X)
        ModelerApplication::Instance()->SetControlValue(HEAD_X, headx);
        ModelerApplication::Instance()->SetControlValue(HEAD_X, headx);
    }

	// 1/50 second update is good enough 0.025
	Fl::add_timeout(1/VAL(FPS), ModelerApplication::RedrawLoop, NULL);
}